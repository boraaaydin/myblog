export const tooltipContent: Record<string, Record<string, string>> = {
  ...
  "vibe-coding": {
    tr: `Vibe coding, yapay zeka destekli kodlama araçlarıyla doğal dil kullanarak kod yazma yaklaşımıdır. Geleneksel kodlamanın aksine, geliştiriciler ne yapmak istediklerini sohbet eder gibi anlatır ve AI bu istekleri koda dönüştürür. Bu yaklaşım hızlı prototipleme ve özellik geliştirme sağlar, ancak context window yönetimi ve görev bölümlendirme gibi stratejiler gerektirir. Claude Code, GitHub Copilot ve Cursor gibi araçlar vibe coding'i destekler.`,
    en: `Vibe coding is an approach to writing code using AI-powered coding tools with natural language. Unlike traditional coding, developers describe what they want to do conversationally, and the AI translates these requests into code. This approach enables rapid prototyping and feature development but requires strategies like context window management and task decomposition. Tools like Claude Code, GitHub Copilot, and Cursor support vibe coding.`,
  },
  "llm": {
    tr: `LLM (Large Language Model), büyük dil modeli anlamına gelir. Milyarlarca parametreyle eğitilmiş yapay zeka modelleridir. Metin anlama, üretme, çevirme ve kod yazma gibi görevlerde kullanılır. GPT-4, Claude ve Llama gibi modeller LLM örnekleridir. LLM'ler doğal dil işleme (NLP) yetenekleriyle AI kodlama asistanlarının temelini oluşturur. Ancak sınırlı context window'ları olduğundan, uzun konuşmalarda performans düşebilir.`,
    en: `LLM (Large Language Model) refers to AI models trained with billions of parameters. They are used for tasks like text understanding, generation, translation, and code writing. Models like GPT-4, Claude, and Llama are examples of LLMs. LLMs form the foundation of AI coding assistants with their natural language processing (NLP) capabilities. However, they have limited context windows, so performance may degrade in long conversations.`,
  },
  "stateless": {
    tr: `Stateless (durumsuz), bir sistemin önceki etkileşimleri hatırlamadığı anlamına gelir. LLM'ler stateless'tır—her sorgulamada önceki konuşma geçmişi tekrar gönderilir. Sistem sizi veya önceki mesajlarınızı doğal olarak hatırlamaz. Bu nedenle her istekte context (bağlam) bilgisi gönderilmelidir. Stateless yapı, context window sınırlamasına neden olur çünkü tüm geçmiş her seferinde işlenir. Web API'lerinde de yaygın bir mimari yaklaşımdır.`,
    en: `Stateless means a system does not remember previous interactions. LLMs are stateless—the entire conversation history is resent with each query. The system does not naturally remember you or your previous messages. Therefore, context information must be sent with each request. The stateless architecture causes context window limitations because all history is processed each time. It is also a common architectural approach in web APIs.`,
  },
  "hallucination": {
    tr: `Halüsinasyon (hallucination), yapay zekanın gerçek olmayan, uydurma veya yanlış bilgi üretmesidir. LLM'ler context window dolduğunda, yeterli bilgi olmadığında veya eğitim verilerinde olmayan konularda halüsinasyon yapabilir. Örneğin, var olmayan bir fonksiyon adı söyleyebilir veya yanlış API kullanımı önerebilir. Halüsinasyonu azaltmak için: context'i temiz tutun, spesifik promptlar kullanın ve AI çıktılarını her zaman doğrulayın.`,
    en: `Hallucination is when AI generates false, fabricated, or incorrect information. LLMs can hallucinate when the context window fills up, when there is insufficient information, or on topics not in their training data. For example, they may mention a non-existent function name or suggest incorrect API usage. To reduce hallucination: keep context clean, use specific prompts, and always verify AI outputs.`,
  },
  "prompt": {
    tr: `Prompt, yapay zeka modeline verilen talimattır. Kodlama asistanlarında, ne yapmak istediğinizi açıkladığınız metin girişidir. İyi bir prompt: net, spesifik, bağlam içerir ve istenen çıktıyı açıkça tanımlar. Kötü prompt: "Bir API yap". İyi prompt: "Express.js kullanarak kullanıcı kaydı için POST /api/register endpoint'i oluştur. Email ve şifre validasyonu yap, bcrypt ile şifrele, JWT token döndür." Prompt mühendisliği (prompt engineering), AI'dan en iyi sonuçları almanın kritik becerisidir.`,
    en: `Prompt is the instruction given to an AI model. In coding assistants, it is the text input where you describe what you want to do. A good prompt: is clear, specific, includes context, and clearly defines the desired output. Bad prompt: "Make an API". Good prompt: "Create a POST /api/register endpoint for user registration using Express.js. Validate email and password, encrypt with bcrypt, return JWT token." Prompt engineering is a critical skill for getting the best results from AI.`,
  },
  "workflow": {
